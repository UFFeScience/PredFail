...
sir.taskid = pea_sir.taskid and
(
(pea_sids.failure_tries = 0 or pea_sids.exitstatus = 0) or
(pea_sim.failure_tries = 0 or pea_sim.exitstatus = 0) or
(pea_sirs.failure_tries = 0 or pea_sirs.exitstatus = 0) or
(pea_simg.failure_tries = 0 or pea_simg.exitstatus = 0) or
(pea_sir.failure_tries = 0 or pea_sir.exitstatus = 0) or
((sir.model1 <> 'WAG+G' AND sir.length <= 250) OR (sir.model2 <> 'JTT+G' and sir.prob2 <= 1000)) 
);
